...
(android_build_verify_so "libzyre.so" &> /dev/null) || {
    ...
    (bash ${ZYRE_ROOT}/builds/qt-android/build.sh) || exit 1
    UPSTREAM_PREFIX=${ZYRE_ROOT}/builds/qt-android/prefix/${TOOLCHAIN_NAME}
    cp -r ${UPSTREAM_PREFIX}/* ${ANDROID_BUILD_PREFIX}
}

##
# Build hydra from local source

(android_build_verify_so "libhydra.so" "libzyre.so" &> /dev/null) || {
    rm -rf "${cache}/hydra"
    (cp -r ../.. "${cache}/hydra" && cd "${cache}/hydra" \
        && make clean && rm configure config.status)
    rm 
    
    export LIBTOOL_EXTRA_LDFLAGS='-avoid-version'
    
    (cd "${cache}/hydra" && ./autogen.sh \
...
android_build_verify_so "libhydra.so" "libzyre.so"
