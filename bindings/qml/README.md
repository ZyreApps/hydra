...
a local installation of the hydra library to link against and include.
...
hydra library is installed locally where Qt can find it for 
running desktop applications that use the library, and tests are run.

You will need to have the hydra C library built and installed on your machine.
...
In order to build for android, the qml_hydra needs access to 
a qt-android build of the hydra library and all its dependencies. 
This can be done one of two ways:

1. Clone the hydra library and all its dependencies from source 
...
`rake android` in the qml_hydra folder.
2. Manually export the `HYDRA_ROOT` environment variable as the path 
to the hydra source code root. Do the same for any other projects 
